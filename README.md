...
python src/main.py
```
**player (must be inside src, test_project too):** 
```bash
python -m player.main test_project
```
...
scenes scripts are inside project_file/scenes/scripts. Background will be set to "" if none are set
```json
{
  "unique_name": "level01",
